...
def rule_based_move(game_array, player):
    ...
    if available_moves:
        ...
        i, j = random_move
        images.append((game_array[i][j][0], game_array[i][j][1], X_IMAGE))
        game_array[i][j] = (game_array[i][j][0], game_array[i][j][1], player, False)
...
def minimax_move(game_array, player):
    ...
    for i in range(len(game_array)):
        for j in range(len(game_array[i])):
            if game_array[i][j][2] == "":
                game_array[i][j] = (game_array[i][j][0], game_array[i][j][1], player, False)
                score, d = minimax(game_array, 0, -math.inf, math.inf, player == 'o')
                game_array[i][j] = (game_array[i][j][0], game_array[i][j][1], '', True)
...
def play_game(screen, player2_type='minimax'):
    ...
    while run:
        ...
        if x_turn:
            game_array = rule_based_move(game_array, 'x')
            x_turn = False
            o_turn = True

        if o_turn:
            if player2_type == 'minimax':
                # print("playing minimax for player 2!")
...
for iteration in range(ITERATIONS):
    wins_losses = calculate_wins_losses(EPISODES, win, player2_type='minimax')
    wins_losses_history.append(wins_losses)
    print(f"Iteration {iteration + 1}:")
    print(f"Player X wins: {wins_losses['player1']}")
    print(f"Player O wins: {wins_losses['player2']}")
