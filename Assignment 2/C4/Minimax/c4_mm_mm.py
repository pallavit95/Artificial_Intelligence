...
def minimax(board, depth, alpha, beta, maximizing_player, piece):
    ...
    if depth == 0 or is_terminal:
        if is_terminal:
            if winning_move(board, piece):  # AI player wins
                return (None, 100000000000000)
            elif winning_move(board, 3-piece):  # Opponent player wins
                return (None, -10000000000000)
            else:  # Game is over, no more valid moves (tie)
                return (None, 0)
        else:  # Depth is zero
            return (None, score_position(board, piece) if maximizing_player else -score_position(board, piece))
...
def evaluate_window(window, piece):
    opponent_piece = 2 if 3-piece==1 else 1
    score = 0
...
def play_game(screen, player2_type='minimax'):
    board = create_board()
    game_over = False
    turn = 0
    global eval_count
    while not game_over:
        if turn == 0:
            eval_count = 0
            col, _ = minimax(board, MAX_DEPTH, -math.inf, math.inf, False, 1)
            if col is not None:
                row = get_next_open_row(board, col)
...
def calculate_wins_losses(episodes, screen, player2_type='minimax'):
    wins_losses = {'player1': 0, 'player2': 0, 'draw': 0}
    for _ in range(episodes):
        result = play_game(screen, player2_type='minimax')
        if result == 1:
...
for iteration in range(ITERATIONS):
    wins_losses = calculate_wins_losses(EPISODES, screen, player2_type='minimax')
